...
func fcmNotify(fcm fcmClient, db *gorm.DB) *worker.Job {
...
func fcmMulticast(fcm fcmClient, db *gorm.DB) *worker.Job {
...
func fcmCleanup(wrkr *worker.Worker, fcm fcmClient, db *gorm.DB) *worker.Job {
	...
	validate := func(ctx context.Context, tokens []models.FCMToken) (int, error) {
		deleted := 0
		msg := firebase.TestMessage(tokenStrings(tokens))
...
func deleteToken(token string, db *gorm.DB) {
