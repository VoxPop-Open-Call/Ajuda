...
import (
	...
	"bitbucket.org/mobinteg/ajuda-mais/src/server"
...
func main() {
	...
	awsClient, err := aws.New(aws.Config{
		...
		MobileDLURI:     conf.MOBILE_DL_URI,
	})
	if err != nil {
...

	{
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
		if err := chatClient.Stop(ctx); err != nil {
			log.Println(err)
		}
	}
