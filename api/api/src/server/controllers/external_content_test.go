...
func (s *ExternalContentControllerTestSuite) TestListSubjects() {
	entries := []models.ExternalContent{
		{
			Title:        "a",
			ArticleUrl:   random.String(100),
			LanguageCode: "en",
			Subject:      "art, science, tours",
		},
		{
			Title:        "b",
			ArticleUrl:   random.String(100),
		...
		},
		{
			...
			State:        "approved",
			LanguageCode: "en",
			Subject:      "precision javelin launching",
		},
	}
	err := s.db.Create(entries).Error
	s.NoError(err)
	...
	result, err := s.external.ListSubjects(ListExternalContentSubjectFilters{}, ctx)
	s.NoError(err)
	s.Equal([]string{
		"art", "science", "tours", "cake making", "precision javelin launching",
	}, result)
}

func (s *ExternalContentControllerTestSuite) TestListSubjectFilter() {
	entries := []models.ExternalContent{
		{
			Title:        "a",
			ArticleUrl:   random.String(100),
	...
	result, err := s.external.List(ListExternalContentFilters{
		Sort:    Sort{"title asc"},
		Subject: "science, tours",
	}, ctx)
	s.NoError(err)
	s.Len(result, 3)
	s.Equal(entries[0].Title, result[0].Title)
	s.Equal(entries[1].Title, result[1].Title)
	s.Equal(entries[3].Title, result[2].Title)
}
