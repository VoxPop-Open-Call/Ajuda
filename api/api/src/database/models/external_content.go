...
type ExternalContent struct {
	...
	Language     Language `json:"language" gorm:"foreignKey:LanguageCode;references:Code;constraint:OnUpdate:CASCADE,OnDelete:CASCADE;"`
}
...
func (s ExternalContentState) Value() (driver.Value, error) {
	if !isValidExternalContentState(string(s)) {
		return "", fmt.Errorf("invalid value for ExternalContentState: %s", s)
	}
	return string(s), nil
}
